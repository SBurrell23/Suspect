...
// a narrow green band that wanders and gets kicked by pressure gusts. Five seconds inside wins.
export default defineMinigame({ id: 'airlockPressurize', label: 'Pressurize Airlock', duration: 'medium', visual: false }, (container, { onComplete, difficulty = 1 }) => {
  ...
  let needle = 0.1, band = 0.5, bandT = 0, bandTarget = 0.5, held = 0, done = false, gustT = 2, gust = 0;
  const bandW = 0.11 / difficulty;
  ...
  return {
    update(dt, t) {
      ...
      gustT -= dt;
      if (gustT <= 0) { gustT = 1.2 + Math.random() * 1.5; gust = (Math.random() - 0.5) * 1.6 * difficulty; }
      gust *= Math.pow(0.02, dt); // decays fast
      needle += ((holding ? 0.7 : -0.6) + gust) * dt;
      needle = clamp(needle, 0, 1);
      bandT -= dt;
      if (bandT <= 0) { bandT = 1.0 + Math.random() * 1.2; bandTarget = 0.15 + Math.random() * 0.7; }
      band += (bandTarget - band) * Math.min(1, dt * 1.4 * difficulty);
      band += Math.sin(t * 2.2) * dt * 0.05;
      const inside = Math.abs(needle - band) < bandW / 2;
      if (inside) held += dt; else held = Math.max(0, held - dt * 0.6);
      if (held >= 5) { done = true; setTimeout(onComplete, 300); }
      ...
      if (Math.abs(gust) > 0.15) label(ctx, gust > 0 ? 'GUST >' : '< GUST', gx + needle * gw, gy - 22, 12, PALETTE.warn);
      ...
      progressBar(ctx, 80, 210, 480, 16, held / 5);
    },
