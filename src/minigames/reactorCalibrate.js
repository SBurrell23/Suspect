...
// Three dials must reach their target values, but every dial nudges the other two (including
// wrap-around), the tolerance is tight, and halfway through the targets shift once.
export default defineMinigame({ id: 'reactorCalibrate', label: 'Calibrate Reactor', duration: 'long', visual: false }, (container, { onComplete, difficulty = 1 }) => {
  ...
  const dials = [0, 1, 2].map((i) => ({ x: 130 + i * 190, y: 200, r: 60, v: Math.random(), target: 0.15 + Math.random() * 0.7 }));
  const coupling = 0.55 * difficulty;
  let drag = null, lastA = 0, okT = 0, done = false, shifted = false, shiftFlash = 0;
  const tol = 0.028;
  ...
  return {
    update(dt) {
      ...
      if (drag && p.down) {
        ...
        dials[(i + 1) % 3].v = clamp(dials[(i + 1) % 3].v + dv * coupling * 0.8, 0, 1);
        dials[(i + 2) % 3].v = clamp(dials[(i + 2) % 3].v - dv * coupling, 0, 1);
      }
      p.consume();
      const okCount = dials.filter((d) => Math.abs(d.v - d.target) < tol).length;
      if (!shifted && okCount >= 2) { shifted = true; shiftFlash = 1; for (const d of dials) d.target = clamp(d.target + (Math.random() - 0.5) * 0.35, 0.1, 0.9); }
      ...
      panelBg(ctx, w, h, shiftFlash > 0 ? 'TARGETS SHIFTED' : 'SET ALL DIALS TO TARGET');
      for (const d of dials) {
        ...
        ctx.beginPath(); ctx.arc(d.x, d.y, d.r + 18, ta - 0.06, ta + 0.06); ctx.strokeStyle = ok ? PALETTE.ok : shiftFlash > 0 ? PALETTE.bad : PALETTE.warn; ctx.lineWidth = 12; ctx.stroke();
      ...
      if (allOk) { ctx.fillStyle = PALETTE.ok; ctx.fillRect(60, 380, 520 * clamp(okT, 0, 1), 6); }
    },
